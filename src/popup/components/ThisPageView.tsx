...
export function ThisPageView({
  ...
  if (loading) {
    return (
      <WorkspaceEmptyState
        loading
        icon={<HugeIcon icon={File01Icon} className="h-5 w-5" />}
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[20px] border border-border bg-card px-4 py-4 shadow-card">
        ...
        <h2 className="mt-2 text-[15px] font-semibold leading-[1.35] text-foreground">
        ...
        <button
          type="button"
          onClick={onAddNote}
          className="mt-4 flex h-[46px] w-full items-center justify-center rounded-[14px] bg-primary text-[14px] font-semibold text-primary-foreground transition-colors hover:bg-primary/80"
        >
          + Add Note
        </button>
      </div>
